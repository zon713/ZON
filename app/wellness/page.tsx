...
export const metadata: Metadata = {
  title: '养生指南｜汇医盟',
  description:
    '从吃饭、睡觉和日常活动这些小事开始，读几篇简单、实用的健康科普文章。',
};
...
export default function WellnessHome() {
  ...
  return (
    <main className="wellness-page min-h-screen text-[#173d35]">
      <WellnessHeader />

      <section className="wellness-hero relative overflow-hidden text-[#f8f1df]">
        <div
          className="hero-paper-texture absolute inset-0"
          aria-hidden="true"
        />
        <div className="relative mx-auto grid max-w-6xl gap-7 px-5 py-9 sm:px-8 sm:py-14 md:grid-cols-[1.2fr_0.8fr] md:items-center">
          ...
          <div className="wellness-hero-note relative hidden rounded-[1.25rem] border border-[#ddc782]/45 px-6 py-5 text-[#173d35] md:block md:px-7 md:py-6">
            <span
              className="absolute -right-1 -top-6 select-none font-serif text-[7rem] leading-none text-[#a87a32]/[0.07]"
              aria-hidden="true"
            >
              读
            </span>
            <p className="relative text-xs font-bold tracking-[0.2em] text-[#9a6d2b]">
              给自己一点时间
            </p>
            <p className="relative mt-3 max-w-sm text-lg font-bold leading-8">
              一篇文章，一件可以从日常做起的小事。
            </p>
            <p className="relative mt-3 text-sm leading-6 text-[#6b766e]">
              内容仅作健康常识参考；持续不适，请向专业医务人员咨询。
            </p>
          </div>
        </div>
      </section>
      ...
      <section
        className="mx-auto max-w-6xl px-5 pb-14 pt-10 sm:px-8 sm:pb-20 sm:pt-14"
        aria-labelledby="guide-title"
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-bold tracking-[0.2em] text-[#9a6d2b]">
              日常养护 · 四个话题
            </p>
            <h2
              id="guide-title"
        ...
        >
          <span className="flex flex-col justify-between px-6 py-7 sm:px-9 sm:py-9">
            <span>
              <span className="flex items-center gap-3 text-sm font-semibold text-[#8b6429]">
                <span className="grid size-10 place-items-center rounded-full bg-[#efe5cf]">
                  <FeaturedIcon className="size-5" aria-hidden="true" />
                </span>
                本期先读{' '}
                <span className="font-normal text-[#87928e]">
                  · {featured.readingTime}
                </span>
              </span>
              <span className="mt-7 block text-[1.8rem] font-bold tracking-[-0.03em] text-[#173d35] sm:text-[2.2rem]">
                {featured.title}
              </span>
              <span className="mt-4 block max-w-lg text-base leading-8 text-[#667a72]">
                {featured.summary}
              </span>
            </span>
          ...
          <span
            className="wellness-feature-art relative hidden min-h-64 overflow-hidden md:grid"
            aria-hidden="true"
          >
            <span className="absolute left-8 top-8 text-xs font-semibold tracking-[0.28em] text-[#5b7969]">
              THE FOUR SEASONS
            </span>
            <span className="relative z-10 place-self-center font-serif text-[5.5rem] tracking-[0.18em] text-[#1f6653]">
              四时
            </span>
            <span className="absolute bottom-8 right-8 text-xs tracking-[0.22em] text-[#7b916f]">
              顺时而养 · 不必勉强
            </span>
          </span>
        </a>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {moreArticles.map((article, index) => {
            ...
            return (
              ...
              >
                <span
                  className="absolute right-5 top-2 font-serif text-[4rem] leading-none text-[#173d35]/[0.06]"
                  aria-hidden="true"
                >
                ...
                >
                  <span className="grid size-10 place-items-center rounded-full bg-[#efe5cf] text-[#8b6429]">
                ...
                <span className="mt-8 block text-2xl font-bold tracking-[-0.025em] text-[#173d35]">
                  {article.shortTitle}
                </span>
                <span className="mt-3 block flex-1 text-sm leading-7 text-[#667a72]">
                  {article.summary}
                </span>
      ...
      <section className="border-y border-[#173d35]/10 bg-[#e9e2d4]">
        <div className="mx-auto grid max-w-6xl gap-5 px-5 py-9 sm:px-8 md:grid-cols-[auto_1fr] md:items-center md:gap-7">
          <span className="grid size-11 place-items-center rounded-full bg-[#173d35] text-[#f4e8c8]">
            <ShieldCheck className="size-5" aria-hidden="true" />
          </span>
          <div>
            <h2 className="text-lg font-bold">读完文章，也记得照顾实际感受</h2>
            <p className="mt-1 text-sm leading-7 text-[#667a72]">
              每篇文章附有参考资料，供了解常识。身体不适持续、加重或影响日常生活时，请及时到正规医疗机构就诊。
            </p>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#faf7ef] px-5 py-7 text-center text-sm text-[#6f817b]">
        <BookOpenText
          className="mx-auto mb-2 size-4 text-[#8b6429]"
          aria-hidden="true"
        />
        汇医盟 · 把健康常识讲得简单一点
      </footer>
